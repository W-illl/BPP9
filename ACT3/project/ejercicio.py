...
def cargar_dataframe():
    '''
    Convierte archivo .csv en dataframe

    return -> df: Dataframe con los datos del csv 
    '''
...
def ingresos():
    '''
    Retorna la suma total de los valores positivos
    ''' 
    mask = df > 0
    return df[mask].sum()
...
def gastos(): 
    '''
    Retorna la suma total de los valores negativos
    ''' 
    mask = df < 0
    return df[mask].sum()
...
def ahorro():
    '''
    Retorna la diferencia entre los valores positivos y valores negativos
    ''' 
    return gastos()+ingresos()

def grafica():
    '''
    Genera y guarda la gráfica sobre la evolución de los ingresos
    '''
    grafica = df_mensual['Ingreso'].plot(kind='line',figsize=(20, 16), fontsize=26).get_figure()
    grafica.savefig(os.path.join('data','plot.png'))

def print_info(df_mensual):
    '''
    Imprime por pantalla información sobre el dataframe cargado
    '''
...
try:
    ...
    print_info(df_mensual)
    grafica()    

                    
except FileNotFoundError:
    print(f'Por favor, comprueba que el existe el archivo csv en la carpeta data')
